...
class Tester {
  ...
  prepareNetwork(component, done) {
    ...
    if (typeof component === 'object' && Array.isArray(component.edges)) {
      ...
      promise = noflo.createNetwork(component, {
        componentLoader: this.loader,
        baseDir: this.baseDir,
        delay: true,
      })
        .then((network) => network.connect());
    } else if (!this.loader.components) {
      // Prepare component loader first
      promise = this.loader.listComponents()
        .then(() => this.prepareNetwork(component, done), done);
    } else if (typeof component !== 'string') {
      // Register the component to be loadable
      this.loader.registerComponent('wrapper', 'Wrapped', this.component);
      promise = this.prepareNetwork('wrapper/Wrapped', done);
    } else {
      promise = this.loader.load(component)
        .then((instance) => {
          if (instance.isSubgraph() && instance.network) {
            // We can use the subraph as-is
            // Subgraph networks are already connected as well
            return Promise.resolve(instance.network);
          }
    ...
    if (done) {
      promise.then((network) => done(null, network), done);
      return;
    }
  ...
  start(done) {
    const promise = this.prepareNetwork(this.component)
      .then((network) => {
        if (typeof (this.options.load) === 'function') {
          this.options.load(null, network);
        }
